import 'dart:io';
import 'package:mason/mason.dart';

Future<void> run(HookContext context) async {
  final logger = context.logger;

  final featureName = context.vars['feature_name'] as String;
  final confirm = context.vars['confirm'] as bool? ?? false;

  final words = _splitWords(featureName);
  if (words.isEmpty) {
    logger.err('❌ Feature name cannot be empty.');
    exit(1);
  }

  final fileName = words.join('_');
  final camelName =
      words.first +
      words.skip(1).map((word) => word[0].toUpperCase() + word.substring(1)).join();

  context.vars = {...context.vars, 'fileName': fileName, 'camelName': camelName};

  final libDir = Directory('${Directory.current.path}/lib');
  final featureDir = Directory('${libDir.path}/features/$fileName');

  if (!featureDir.existsSync()) {
    logger.err(
      '❌ lib/features/$fileName doesn\'t exist — nothing to remove.',
    );
    exit(1);
  }

  // Best-effort safety net: warn about anything OUTSIDE this feature's own
  // folder that still references it (e.g. core/services/session_manager.dart
  // depending on auth's AuthEntity), so a confirmed deletion doesn't
  // silently leave a dangling import somewhere unexpected. Doesn't block —
  // `confirm` is the actual gate — just makes the blast radius visible
  // before it happens.
  //
  // Excludes the files post_gen.dart itself auto-cleans (routes,
  // injection_container.dart) — those aren't "manual fixing", they're
  // handled by this very brick a moment later, so warning about them here
  // would just be noise.
  const autoFixedFiles = {
    'routes/route_names.dart',
    'routes/route_paths.dart',
    'routes/app_router.dart',
    'core/di/injection_container.dart',
  };

  final externalRefs = _findExternalReferences(
    libDir,
    'features/$fileName/',
    excludeDir: featureDir,
  ).where((path) => !autoFixedFiles.any((f) => path.endsWith(f))).toList();
  if (externalRefs.isNotEmpty) {
    logger.warn(
      '⚠️  These files reference features/$fileName/ and will need manual '
      'fixing after this feature is removed:',
    );
    for (final path in externalRefs) {
      logger.warn('   - $path');
    }
  }

  if (!confirm) {
    logger.err(
      '❌ Not deleting anything — pass --confirm to actually remove the '
      '"$fileName" feature. This will delete lib/features/$fileName/ '
      '(every page it contains) and all of its routes/DI wiring.',
    );
    exit(1);
  }
}

// Splits arbitrary input (snake_case, camelCase, PascalCase,
// space/hyphen-separated, or any mix) into lowercase words.
List<String> _splitWords(String text) {
  final normalized = text.trim().replaceAll(RegExp(r'[\s\-]+'), '_');
  final withBoundaries = normalized.replaceAllMapped(
    RegExp(r'(?<=[a-z0-9])(?=[A-Z])'),
    (match) => '_',
  );
  return withBoundaries
      .split('_')
      .where((word) => word.isNotEmpty)
      .map((word) => word.toLowerCase())
      .toList();
}

/// Scans every .dart file under [libDir] (except [excludeDir]) for a
/// literal occurrence of [needle], returning the relative paths of any
/// that contain it.
List<String> _findExternalReferences(
  Directory libDir,
  String needle,
  {required Directory excludeDir}
) {
  final matches = <String>[];
  final excludePath = excludeDir.absolute.path;

  for (final entity in libDir.listSync(recursive: true)) {
    if (entity is! File || !entity.path.endsWith('.dart')) continue;
    if (entity.absolute.path.startsWith(excludePath)) continue;

    final content = entity.readAsStringSync();
    if (content.contains(needle)) {
      matches.add(entity.path.replaceAll('\\', '/'));
    }
  }
  return matches;
}
